...
syn keyword perlStatementFiledesc	binmode close closedir eof fileno getc lstat print printf read readdir readline readpipe rewinddir say select stat tell telldir write nextgroup=perlFiledescStatementNocomma skipwhite
syn keyword perlStatementFiledesc	fcntrl flock ioctl open opendir read seek seekdir sys sysopen sysread sysseek syswrite truncate nextgroup=perlFiledescStatementComma skipwhite
syn keyword perlStatementVector		vec
syn keyword perlStatementFiles		ch chdir chmod chown chroot glob link mkdir readlink rename rmdir symlink umask unlink utime
...
syn keyword perlStatementInclude	use no nextgroup=perlStatementInclude2 skipwhite
syn keyword perlStatementInclude2	attributes attrs autodie autouse parent base bigint bignum bigrat blib bytes charnames constant diagnostics encoding encoding::warnings feature fields filetest if integer less lib locale mro open ops overload overloading re sigtrap sort strict subs threads threads::shared utf8 vars version vsmish warnings warnings::register contained
...
syn keyword perlStatementNetwork	endhostent endnetent endprotoent endservent gethostent getnetent getprotoent getservent sethostent setnetent setprotoent setservent gethostbyaddr gethostbyname getnetbyaddr getnetbyname getprotobyname getprotobynumber getservbyname getservbyport
...
syn region perlStatementIndirObjWrap matchgroup=perlStatementIndirObj start="\%(\<\%(map\|grep\|sort\|printf\=\|say\|system\|exec\)\>\_s*\)\zs{" end="}" transparent extend
...
syn region perlSpecialMatch	start="\[\^\=" skip="\\\]\|\[:\h*:\]" end="\]" contained
...
hi def link perlStatementInclude2	perlStatementInclude
